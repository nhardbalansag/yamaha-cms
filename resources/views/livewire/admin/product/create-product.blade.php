            ...
            <form wire:submit.prevent="createProduct" enctype="multipart/form-data">
                <div class="sm:rounded-md sm:overflow-hidden">
                    ...
                    Upload file
                     ...
                     <label class="ml-4 cursor-pointer bg-white hover:bg-gray-100 w-1/4 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow-xs ">
                    ...
                    <div class="px-4 py-5 bg-white sm:p-6">
                        <div class="col-span-3 sm:col-span-3 w-1/4 mb-4">
                            <label for="country" class="block text-sm font-medium leading-5 text-gray-700 capitalize">category</label>
                            <select wire:model.prevent ="product_category_id" id="country" class="mt-1 block form-select w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5">
                                @forelse($productCategory as $category)
                                     <option value = "{{$category->id}}" >{{$category->title}}</option>
                                    @empty
                                @endforelse
                            </select>
                              @error('product_category_id') <span class="error text-red-600">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-span-3 sm:col-span-3 w-1/4 mb-4">
                            ...
                            <select wire:model.prevent ="specification_id" id="country" class="mt-1 block form-select w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5">
                              ...
                              @error('specification_id') <span class="error text-red-600">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-span-3 sm:col-span-3 w-1/4 mb-4">
                            <label for="country" class="block text-sm font-medium leading-5 text-gray-700 capitalize">color</label>
                            <select wire:model.prevent ="colors_type_id" id="country" class="mt-1 block form-select w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5">
                                @forelse($colorCategory as $category)
                                     <option value = "{{$category->id}}" >{{$category->title}}</option>
                                    @empty
                                @endforelse
                            </select>
                              @error('colors_type_id') <span class="error text-red-600">{{ $message }}</span> @enderror
                        </div>
                        <div class="grid grid-cols-3 gap-6 py-2">
                            <div class="col-span-3 sm:col-span-2">
                                ...
                                <input wire:model='price' id="company_website" class="form-input flex-1 block w-full rounded-none rounded-r-md transition duration-150 ease-in-out sm:text-sm sm:leading-5">
                                </div>
